...
workbook_file = st.sidebar.file_uploader(
    "Carregar planilha do Scanner (.xlsx)",
    type=["xlsx"]
)
prices_file = st.sidebar.file_uploader(
    "Carregar preços OHLCV (.csv)",
    type=["csv"]
)

if workbook_file is None or prices_file is None:
    st.info("Carregue a planilha do Scanner e o arquivo CSV de preços na barra lateral para iniciar.")
    st.stop()

@st.cache_data
def load(workbook_file, prices_file):
    watch=pd.read_excel(workbook_file,sheet_name="Watchlist Atual")
    queue=pd.read_excel(workbook_file,sheet_name="Fila de Alertas")
    hist=pd.read_excel(workbook_file,sheet_name="Historico")
    px=pd.read_csv(prices_file,parse_dates=["date"])
...
try:
    watch,queue,hist,px=load(workbook_file,prices_file)
